...
Game* game_create(char * name_player) {
    int i;
    Game* game = NULL;
    ...
    if (player_set_id(game->player, 1) == ERROR) return NULL; /*En esta primera iteración, suponemos que hay un solo jugador*/
    if (player_set_name(game->player, name_player) == ERROR) return NULL;
    for (i = 0; i < MAX_OBJECT; i++) {
        game->object[i] = NULL;
    }
    game->die = die_create();
    if (game->die == NULL) return NULL;
...
Game* game_create_from_file(char* filename, char* name_player) {
    Game* game = NULL;
    game = game_create(name_player);
    if (game == NULL) return NULL;
...
STATUS game_update(Game* game, Comand *comand) {
    ...
    switch (comand_get_comand(comand)) {
        ...
        case TAKE:
            ...
            return game->resultado_comando;
        case LEAVE:
            game->resultado_comando = game_callback_leave_object(game);
            return game->resultado_comando;
        case ROLL:
...
STATUS game_callback_take_object(Game* game, Comand *cmd) {
    ...
    if (player_get_object_id(game->player) != NO_ID) { /*Si el jugador tiene tiene un objeto*/
        if (space_take_object(game_get_space(game, space_id), object_id) == ERROR) return ERROR;
        space_leave_object(game_get_space(game, space_id), player_get_object_id(game->player));
        player_set_object_id(game->player, object_id);
        return OK;
    } else { /*Si el jugador tiene no tiene ningún objeto*/
        if (space_take_object(game_get_space(game, space_id), object_id) == ERROR) return ERROR;
        player_set_object_id(game->player, object_id);
        return OK;
    }
...
strcpy (game->ultima_descripcion_pedida, space_get_description(game, game_get_space(game_get_player_location(game)));
return OK;
}
for (i = 0; game->object[i] != NULL && strcasecmp(object_get_name(game->object[i]), object_name) != 0 && i < MAX_OBJECT; i++);
if (i > MAX_OBJECT) return ERROR;
if (game->object[i] == NULL) return ERROR;
strcpy (game->ultima_descripcion_pedida, space_get_description(game, game->object[i]));
return OK;
}
...
if (strcasecmp(name, "n")==0 || strcasecmp(name, "north")==0){
  return game_callback_previous(game);
}
if (strcasecmp(name, "s")==0 || strcasecmp(name, "south")==0){
  return game_callback_following(game);
}
if (strcasecmp(name, "w")==0 || strcasecmp(name, "west")==0){
  return game_callback_oca_previous(game);
}
if (strcasecmp(name, "e")==0 || strcasecmp(name, "east")==0){
